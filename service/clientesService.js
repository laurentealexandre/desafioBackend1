...
const update = async (clientes) => {
    ...
    const isOk = await (await connection).execute(query, [clientes.nome, clientes.sobrenome, clientes.email, clientes.idade, clientes.id]);
    return isOk[0].affectedRows === 1;
}

const save = async (clientes) => {
    const query = 'INSERT INTO clientes(nome, sobrenome, email, idade) VALUES (?, ?, ?, ?)';
    const isOk = await (await connection).execute(query, [clientes.nome, clientes.sobrenome, clientes.email, clientes.idade]);
    return isOk[0].affectedRows === 1;
}
