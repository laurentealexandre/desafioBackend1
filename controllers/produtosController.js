...
const update = async (request, response) => {
    ...
    return result ?
        response.status(200).json() : response.status(400).json({ "[ERROR/SERVER]": "Falha ao atualizar produto" });
};
...
const remove = async (request, response) => {
    ...
    return result ?
        response.status(200).json() : response.status(400).json({ "[ERROR/SERVER]": "Falha ao remover produto" });
};
